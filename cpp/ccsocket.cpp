...
Socket::Socket(int, SOCKET sockfd) : sockfd_(sockfd) { startup(); }
...
int Socket::bind(int port) {
    ...
    // assigns the address specified by addr to sockfd (returns -1 on error, 0
    // otherwise)
...
int Socket::connect(const string& host, int port) {
    ...
    SOCKADDR_IN addr;
    if (setAddress(addr, host, port) < 0) return UnknownHost;
    // connects sockfd to the address specified by addr (returns -1 on error, 0
    // otherwise)
...
void Socket::shutdownInput() { ::shutdown(sockfd_, 0); }

void Socket::shutdownOutput() { ::shutdown(sockfd_, 1 /*SD_SEND*/); }
...
int ServerSocket::bind(int port, int backlog) {
    ...
    // listen s'applique seulement aux sockets de type SOCK_STREAM ou
    // SOCK_SEQPACKET.
...
int ServerSocket::setTcpNoDelay(bool state) {
    // turn off TCP coalescence for INET sockets (useful in some cases to avoid
    // delays)
    int set = state;
    return ::setsockopt(sockfd_, IPPROTO_TCP, TCP_NODELAY, (char*)&set,
                        sizeof(int));
}
...
SocketBuffer::~SocketBuffer() { delete in_; }

void SocketBuffer::setReadSeparator(int separ) { insep_ = separ; }

void SocketBuffer::setWriteSeparator(int separ) { outsep_ = separ; }
