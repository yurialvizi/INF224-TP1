...
class Socket {
   public:
    ...
    /// @note TCP/IP sockets do not preserve record boundaries, see
    /// SocketBuffer.
    ...
    /// By default, this function blocks the caller until thre is availbale
    /// data.
    /// @return the number of bytes that were received, or 0 or shutdownOutput()
    /// was called on the other side, or Socket::Failed (-1) if an error
    /// occured.
...
class ServerSocket {
   public:
    ...
    /// Accepts a new connection request and returns a socket for exchanging
    /// data with this client. This function blocks until there is a connection
    /// request.
...
class SocketBuffer {
   public:
     ...
     * @note the separator (eg \n) is counted in the value returned by
     * readLine().
     */
